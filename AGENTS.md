...
- Do not run local Settings or Dock UI suites unless the user explicitly requests them: they alter the user's Dock and open apps. `scripts/release.sh` leaves them off by default; `DOCKMINT_RUN_LOCAL_UI_CHECKS=1` is only for an explicitly requested run. Hosted CI owns native release-package verification.
- The local pre-tag gate owns deterministic and native validation. The tag workflow starts package builds after tag-contract validation and does not repeat the same XCTest matrix before compiling signed packages.
